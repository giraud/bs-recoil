...
let filteredTodoListState = Recoil.Selector.value(
...
let todoListStatsState = Recoil.Selector.value(
...
module TodoItem = {
  ...
  let make = (~item) => {
    ...
    let index =
      todoList->Belt.Array.getIndexBy(listItem => listItem === item)->Belt.Option.getWithDefault(-1)

    let editItemText = event => {
      let text: string = (event->ReactEvent.Form.target)["value"]
      setTodoList(._ =>
        todoList->Belt.Array.mapWithIndex((i, item) => i == index ? {...item, text: text} : item)
      )
    }
    ...
    let deleteItem = _ =>
      setTodoList(._ => todoList->Belt.Array.keepWithIndex((_, i) => i != index))
...
module TodoStats = {
  @react.component
  let make = () => {
    let (totalNum, totalCompletedNum, totalUncompletedNum, percentCompleted) = Recoil.useValue(
      todoListStatsState,
    )
    ...
    <ul>
      ...
      <li>
        {("Percent completed: " ++ Js.Float.toString(formattedPercentCompleted))->React.string}
      </li>
    </ul>
  }
}
