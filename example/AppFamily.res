...
let listFamily =
  Recoil.Atom.value(~key="List", ~default=([]: array<string>), ())->Recoil.Atom.Family.make
...
module List = {
  ...
  let make = (~id, ~items) => {
    ...
    <div>
      ...
      <ul>
        {items->Belt.Array.map(item => <Item key=item item atom=rwItemSelector />)->React.array}
      </ul>
    </div>
  }
}
